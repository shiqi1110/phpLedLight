...
//总数
$sql = "SELECT count(*) AS c FROM {$pre_}tag";
$count = sqlOne($sql);
...
//查询数据
$sql = "SELECT * FROM {$pre_}tag LIMIT $start,$limit";
$tag_list = sqlAll($sql);
...
if(isset($_GET['tag_id']))
{
    ...
    if($tag && $cate)
    {
        @show_msg('删除标签成功','tag_list.php');
        exit;
    }else{
        @show_msg('删除标签失败','tag_list.php');
        exit;
    }
}
...
<div class="btn-toolbar">
    <button class="btn btn-primary" onClick="location='tag_add.php'"><i class="icon-plus"></i>标签列表</button>
  <div class="btn-group">
  </div>
</div>
<div class="well">
    <table class="table">
      ...
      <?php foreach($tag_list as $item){?>
        <tr>
          ...
          <td>
              <a href="tag_edit.php?tag_id=<?php echo $item['tag_id'];?>"><i class="icon-pencil"></i></a>
              <a onclick="delete_tag('<?php echo $item['tag_id'];?>')" href="javascript:void(0)"><i class="icon-remove"></i></a>
          </td>
        </tr>
        <?php }?>
      </tbody>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <script type="text/javascript">
        ...
        function delete_tag(tag_id){
          if(confirm('是否确认删除')){
            window.location.href="tag_list.php?tag_id="+tag_id;
          }
        }
    </script>

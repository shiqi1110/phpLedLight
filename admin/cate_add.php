...
if($_POST)
{
    ...
    foreach ($tag_list as $item) {
        if($item['cate_name'] == $_POST['cate_name']){
            show_msg('标签名已存在',"cate_edit.php?cate_id=$cate_id");
            exit;
        }
    }

    $insertId = insertData("cate",$data);

    if($insertId){
        @show_msg('插入分类成功','cate_list.php');
        exit;
    }else{
        @show_msg('插入分类失败','cate_add.php');
        exit;
    }
}
    ...
    <div id="myTabContent" class="tab-content">
      <div class="tab-pane active in" id="home">
        <form class="cate_add" action="#" method="post" enctype="multipart/form-data">
            <label>分类名称</label>
            <input type="text" datatype="s4-25" value="" name="cate_name" class="input-xxlarge">
            <label></label>

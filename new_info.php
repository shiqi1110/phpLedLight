...
$tag_list = findAll('tag',"cate_id=3");

$tag_id = isset($_GET['tag_id']) ? $_GET['tag_id'] : 10;
...
//上一篇
$sql = "SELECT art_id FROM {$pre_}article WHERE cate_id>2 AND art_id<$art_id ORDER BY art_id DESC LIMIT 1";
$prev = sqlOne($sql);
//下一篇
$sql = "SELECT art_id FROM {$pre_}article WHERE cate_id<4 AND art_id > $art_id LIMIT 1";
$next = sqlOne($sql);
        ...
        <div class="cms-g">
            <div class="am-hide-sm-only am-u-md-3 am-u-lg-3">
                <div class="com-nav-left">
                    ...
                    <ul>
                         <li class="<?php echo $tag_id==$item['tag_id'] ? "on":"";?>"><a href="new_info.php?tag_id=<?php echo $item['tag_id'];?>&art_id=<?php echo $res['art_id'];?>"><?php echo $item['tag_name'];?></a></li>
            ...
            <div class="am-u-sm-12 am-u-md-9 am-u-lg-9">
                ...
                <div class="com-info-page">
                    <?php if(!is_null($prev)){?>
                        <a href="new_info.php?tag_id=<?php echo $tag_id;?>&art_id=<?php echo $prev['art_id'];?>">上一篇</a>
                    <?php }else{ ?><a>没有上一篇了</a><?php }?>  
                    <?php if(!is_null($next)){?><a href="new_info.php?tag_id=<?php echo $tag_id;?>&art_id=<?php echo $next['art_id'];?>">下一篇</a>
                    <?php }else{ ?>
                        <a>没有下一篇了</a>
                    <?php }?>
                </div>
